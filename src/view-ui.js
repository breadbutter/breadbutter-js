...
const cleanContinueWithSignInHolder = function() {
    let content = document.body.querySelector('.breadbutter-ui-continue-with-logged-in');
    if (content) {
        content.remove();
    }
};

const getContinueWithSignInHolder = function(options) {
    cleanContinueWithSignInHolder();
    let popup = document.createElement('div');
    popup.classList.add('breadbutter-ui-continue-with-logged-in');
...
const getContinueWithSignInWidget = function(profile, suggested) {
    ...
    `;
    let all_cls = ['bb-top-left', 'bb-top-right', 'bb-bottom-left', 'bb-bottom-right'];
    widget.querySelector('.breadbutter-ui-profile-widget').onclick = function() {
        widget.classList.toggle('clicked');
        if (widget.classList.contains('clicked')) {
            let boundry = widget.getBoundingClientRect();
            let max_width = window.innerWidth;
            let max_height = window.innerHeight;
            let vertical = 'top';
            let horizontal = 'left'
            if (boundry.x > max_width / 2) {
                horizontal = 'right';
            }
            ...
            widget.classList.remove(...all_cls);
        }
    };
    return widget;
};
...
const getContentGatingCoverPage = (options) => {
    ...
    let more_content = popup.querySelector('.breadbutter-ui-content-gating-more-content');
    let text_3 = document.createElement('span');
    text_3.innerHTML = locale.POPUP.TEXT_3;
    let more_button = document.createElement('span');
    more_button.classList.add('breadbutter-floating-more');
    more_button.innerHTML = locale.POPUP.MORE;
    more_button.addEventListener('click', function() {
       text_3.innerHTML = locale.POPUP.TEXT_3_2;
       this.remove();
    });
...
const getContactUsIcon = function(options, message) {
    let icon = view.addView(CONTACT_US_ICON_ID);
